...
export default function StockChart({ stockSymbol, onChartLoaded }: StockChartProps) {
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      try {
        const response = await fetch(`/backend/stock_data/${stockSymbol}_data.json`)
        ...
        if (response.ok) {
          ...
          const prediction = data.prediction.data.map(
            (item: { date: string; price: string }) => ({
              date: item.date,
              price: Number(item.price),
            })
          ...
          const upperBound = data.prediction.upper_bound.map(
            (item: { date: string; price: string }) => ({
              date: item.date,
              price: Number(item.price),
            })
          ...
          const lowerBound = data.prediction.lower_bound.map(
            (item: { date: string; price: string }) => ({
              date: item.date,
              price: Number(item.price),
            })
          ...
          if (chart.current) {
            ...
            const formattedHistoricalData = historical.map((item: ChartDataPoint) => ({
              time: item.date,
              value: item.price,
            }))
            ...
            if (prediction.length > 0) {
              ...
              const formattedPredictionData = prediction.map((item: ChartDataPoint) => ({
                time: item.date,
                value: item.price,
              }))
              ...
              if (upperBound.length > 0) {
                ...
                const formattedUpperBoundData = upperBound.map((item: ChartDataPoint) => ({
                  time: item.date,
                  value: item.price,
                }))
              ...
              if (lowerBound.length > 0) {
                ...
                const formattedLowerBoundData = lowerBound.map((item: ChartDataPoint) => ({
                  time: item.date,
                  value: item.price,
                }))
